...
public class KindContractDao extends Dao<KindContract> {
    ...
    public void onCreate() {
        try{
            connection.prepareStatement("create table IF NOT EXISTS Vid_dog(Id_vid_dog serial PRIMARY KEY,Vid_dog varchar(50),Id_tip_dog integer NOT NULL, FOREIGN KEY (Id_tip_dog) REFERENCES Tip_dog (Id_tip_dog))").execute();
        }
        catch (Exception e){}
    }
    ...
    public Integer Add(KindContract value) {
        try{
            PreparedStatement statement = connection.prepareStatement("INSERT INTO public.vid_dog(vid_dog, id_tip_dog) VALUES (?, ?) RETURNING id_tip_dog");
